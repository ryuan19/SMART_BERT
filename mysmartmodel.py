...
class SMARTLoss(nn.Module):
    ...
    def forward(self, embed: Tensor, state: Tensor) -> Tensor: #need param for which task
        noise = torch.randn_like(embed, requires_grad=True) * self.noise_var
        # Indefinite loop with counter
        for i in count():
            # Compute perturbed embed and states
            embed_perturbed = embed + noise
            state_perturbed = self.eval_fn(embed_perturbed) #NEEDA CHANGE THE INPUT
            # TODO - DESIGN eval function to take in embed_perturb
            # TODO - use Adam?
            ...
            # Compute noise gradient ∂loss/∂noise.. loss using noise to compute
            noise_gradient, = torch.autograd.grad(loss, noise)#, allow_unused=True) #ADDED
